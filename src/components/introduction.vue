<template>
    <div v-stoprightmouse @click.self="closeIntroduction" class="w-[100vw] h-[100vh] fixed left-0 top-0 bg-[rgba(39,39,39,0.9)] flex flex-wrap items-center justify-center">
        <div id="main" class="relative w-[80vw] h-[60vw] md:w-[600px] md:h-[450px] bg-[#FAF3E0] rounded-xl flex flex-col items-center justify-center gap-[10px]">
            <img @click="closeIntroduction" class="absolute top-0 right-0 m-2 h-[20px] md:h-[35px] z-[10] cursor-pointer" src="@/assets/img/error.png" alt="">
            <div class="w-full h-full overflow-hidden">
                    ...
                    <div :style="childStyle" class=" h-full flex flex-col items-center justify-center">
                        <div class="w-full h-4/5 overflow-x-hidden overflow-y-auto">
                            ...
                            <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                                <span class="font-semibold">棋盤與地雷數量:</span>棋盤是一個矩形網格，每個格子可能藏有地雷或是安全的。地雷的數量會在遊戲開始時確定。
                            </div>
                            <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                                <span class="font-semibold">點擊格子:</span>點擊後，安全格子會顯示一個數字，表示與該格子相鄰的八個格子中有多少個地雷。點擊後若遊戲結束，表示踩到地雷
                            </div>
                            <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                                <span class="font-semibold">標記地雷:</span>玩家可以右鍵點擊（或使用標記功能）將格子標記為地雷，避免誤點。通常用旗子表示。
                            </div>
                            <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                                <span class="font-semibold">連鎖反應:</span>如果點擊的格子周圍沒有地雷(顯示數字為「0」)，會自動揭開相鄰的安全格子，直到顯示非零數字的格子為止。
                            </div>
                            <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                                <span class="font-semibold">勝利條件:</span>成功揭開所有非地雷的格子，並正確標記所有地雷，即可贏得遊戲。
                            </div>
                            <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                                <span class="font-semibold">失敗條件:</span>點擊到地雷即遊戲結束。
                            </div>
                        </div>
                    </div>
                    <div :style="childStyle" class=" h-full flex flex-col items-center justify-center">
                        ...
                        <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                            <span class="font-semibold">邏輯推理:</span>利用格子上的數字推斷地雷的位置。例如，數字「1」表示該格子周圍只有一個地雷。
                        </div>
                        <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                            <span class="font-semibold">標記功能:</span>當確定某個格子是地雷時，立即標記以減少風險。
                        </div>
                        <div class="w-full py-[1px] md:py-1 px-2 md:px-4 text-xs md:text-lg">
                            <span class="font-semibold">注意邊界:</span>靠近棋盤邊緣的格子可能會增加判斷的難度。
                        </div>
                    </div>
                </div>
                ...
                <div class="absolute w-auto bottom-0 left-1/2 -translate-x-1/2 p-1 flex flex-wrap items-center justify-center">
                    <template v-for="item in 3" :key="item">
                        <img @click="shift(item)" class="h-[20px] md:h-[35px] cursor-pointer" src="@/assets/img/dot.png" alt="">
                    </template>
                </div>
            </div>
        </div>
